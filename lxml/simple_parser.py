...
class Downloader():
    '''
    Class to retrive HTML code
    and binary files from a speficif website
    '''
...
class xkcdParser(Downloader):
    ...
    def get_last_comic_nr(self):
        try:
            self.last_comic_nr = re.search(r"http://xkcd.com/(\d+)", self.contents).group(1) # saying we want more digits
